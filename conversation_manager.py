...
class ConversationManager:
    ...
    def process_message(self, message, context=None):
        ...
        if context:
            # Map external context to internal context variables
            if 'location' in context:
                self.conversation_context['user_location'] = context['location']
            if 'procedures' in context:
                self.conversation_context['required_procedures'] = context['procedures']
            if 'insurance' in context:
                self.conversation_context['insurance_plan'] = context['insurance']
    ...
    def _provide_direct_analysis(self, message_lower):
        ...
        if hospitals:
            response_msg = self._format_professional_comparison(hospitals, procedures, location, insurance)
    ...
    def _handle_location_procedure_inquiry(self, entities, message_lower):
        ...
        if location and procedures:
            ...
            if hospitals:
                # Use professional formatting for complete analysis
                insurance = self.conversation_context.get('insurance_plan')
                response_msg = self._format_professional_comparison(hospitals, procedures, location, insurance)
    ...
    def _format_professional_comparison(self, hospitals, procedures, location, insurance=None):
        """Format hospital comparison in professional tabular format"""
        if not hospitals:
            return "No hospital data available for comparison."
        
        # Header section
        response_msg = f"🏥 **HEALTHCARE PRICE ANALYSIS - {location.upper()}**\n"
        response_msg += f"📋 **Procedure(s):** {', '.join(procedures)}\n"
        if insurance:
            response_msg += f"🛡️ **Insurance:** {insurance}\n"
        response_msg += f"📅 **Report Date:** {datetime.now().strftime('%B %d, %Y at %I:%M %p')}\n\n"
        
        # Summary section
        cheapest = hospitals[0]
        most_expensive = hospitals[-1]
        total_savings = most_expensive['total_cash_cost'] - cheapest['total_cash_cost']
        
        response_msg += "📊 **EXECUTIVE SUMMARY**\n"
        response_msg += "─" * 50 + "\n"
        response_msg += f"🏆 **Best Value:** {cheapest['hospital']['name']}\n"
        response_msg += f"💰 **Lowest Price:** ${cheapest['total_cash_cost']:,}\n"
        response_msg += f"💸 **Highest Price:** ${most_expensive['total_cash_cost']:,}\n"
        response_msg += f"💡 **Maximum Savings:** ${total_savings:,} ({((total_savings/most_expensive['total_cash_cost'])*100):.1f}% savings)\n\n"
        
        # Detailed comparison table
        response_msg += "📋 **DETAILED HOSPITAL COMPARISON**\n"
        response_msg += "─" * 80 + "\n"
        
        # Table header
        response_msg += f"{'RANK':<4} {'HOSPITAL NAME':<25} {'CASH PRICE':<12} {'RATING':<8} {'WAIT TIME':<10} {'EMERGENCY':<9}\n"
        response_msg += "─" * 80 + "\n"
        
        # Table rows
        for rank, hospital_data in enumerate(hospitals[:5], 1):
            hospital = hospital_data['hospital']
            name = hospital['name'][:22] + '...' if len(hospital['name']) > 25 else hospital['name']
            price = f"${hospital_data['total_cash_cost']:,}"
            rating = f"⭐{hospital['rating']}"
            wait_time = f"{hospital['average_wait_time']}min"
            emergency = "🚨 YES" if hospital['emergency'] else "❌ NO"
            
            response_msg += f"{rank:<4} {name:<25} {price:<12} {rating:<8} {wait_time:<10} {emergency:<9}\n"
        
        response_msg += "─" * 80 + "\n\n"
        
        # Procedure breakdown for top hospital
        if cheapest['procedures']:
            response_msg += f"💊 **PROCEDURE BREAKDOWN - {cheapest['hospital']['name'].upper()}**\n"
            response_msg += "─" * 60 + "\n"
            response_msg += f"{'PROCEDURE':<20} {'BASE PRICE':<12} {'CASH PRICE':<12} {'SAVINGS':<10}\n"
            response_msg += "─" * 60 + "\n"
            
            for proc in cheapest['procedures']:
                proc_name = proc['procedure'][:17] + '...' if len(proc['procedure']) > 20 else proc['procedure']
                base_price = f"${proc['base_price']:,}"
                cash_price = f"${proc['cash_price']:,}"
                savings = f"${proc['savings_cash']:,}"
                
                response_msg += f"{proc_name:<20} {base_price:<12} {cash_price:<12} {savings:<10}\n"
            
            response_msg += "─" * 60 + "\n\n"
        
        # Insurance analysis section
        if insurance:
            response_msg += f"🛡️ **INSURANCE IMPACT ANALYSIS - {insurance.upper()}**\n"
            response_msg += "─" * 50 + "\n"
            response_msg += "✅ **Benefits:**\n"
            response_msg += "   • Typically covers 70-90% of procedure costs\n"
            response_msg += "   • Reduces out-of-pocket expenses significantly\n"
            response_msg += "   • Access to network pricing discounts\n"
            response_msg += "   • Predictable copay structure\n\n"
            
            response_msg += "📞 **Next Steps:**\n"
            response_msg += "   • Contact your insurance for exact coverage details\n"
            response_msg += "   • Verify the hospital is in your network\n"
            response_msg += "   • Ask about pre-authorization requirements\n\n"
        
        # Action items
        response_msg += "🎯 **RECOMMENDED ACTIONS**\n"
        response_msg += "─" * 40 + "\n"
        response_msg += "1️⃣ **Call the top-ranked hospital for scheduling**\n"
        response_msg += f"   📞 {cheapest['hospital']['phone']}\n"
        response_msg += f"   📍 {cheapest['hospital']['address']}\n\n"
        response_msg += "2️⃣ **Verify insurance coverage and costs**\n"
        response_msg += "3️⃣ **Compare wait times if urgent**\n"
        response_msg += "4️⃣ **Check online reviews and ratings**\n\n"
        
        response_msg += "❓ **Need more help?** Ask me to:\n"
        response_msg += "   • Find emergency hospitals\n"
        response_msg += "   • Compare different procedures\n"
        response_msg += "   • Analyze other locations\n"
        response_msg += "   • Get directions to hospitals"
